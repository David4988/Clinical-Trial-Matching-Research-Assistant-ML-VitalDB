...
- `model/anomaly_results.csv`, `model/experiment_summary.json` — Model A
- `ablation/` — comparison CSV, Model B results, ablation summary
- `xai/evidence_cases.json` — 14 structured evidence objects
...
.venv/bin/python -m pytest -q            # 182 tests, offline, no network
...
- **Coverage is still a selection confound.** It no longer feeds the model, but the 70%
  ...
  in excluded windows.
- **The evidence layer's `*_changed` flags measure mean shifts**, while Model B mostly
  keys on *within-window dispersion* (flagged windows have hr_range 21.5 vs 8.0, rr_std
  3.37 vs 0.0, while |hr_delta| is 1.86 vs 1.95 — indistinguishable). The evidence
  objects carry `std`/`min`/`max` so the information is present, but a dispersion flag
  should be added before the explanation layer is built.
